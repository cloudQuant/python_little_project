...
class GarchModel:
    ...
    def __init__(self, ticker, repo, use_new_data=False, model_directory=settings.model_directory):
    ...
    def wrangle_data(self, n_observations):
        ...
        if self.use_new_data:
            av = AlphaVantageAPI(settings.alpha_api_key)
            df = av.get_daily(self.ticker)
            self.repo.insert_table(self.ticker, df, "append")
    ...
    def fit(self, p, q):
        ...
        self.model = arch_model(self.data ** 2, vol='Garch', p=p, q=q).fit()
    ...
    def __clean_prediction(self, prediction):
        ...
        start_date = prediction.index[-1] + pd.Timedelta(days=1)

        # Create date range
        prediction_dates = pd.date_range(start=start_date, periods=5, freq='B')

        # Create prediction index labels, ISO 8601 format
        prediction_index = prediction_dates.strftime('%Y-%m-%dT%H:%M:%S')

        # Extract predictions from DataFrame, get square root
        prediction = prediction.iloc[-1] ** 0.5

        # Combine `data` and `prediction_index` into Series
        prediction.index = prediction_index
        result = pd.Series(prediction, index=prediction_index)
    ...
    def dump(self):
        ...
        filename = f"{self.ticker}__{timestamp}.pkl"
    ...
    def load(self):
        ...
        pattern = os.path.join(self.model_directory, f"{self.ticker}__*.pkl")
        ...
        try:
            ...
            print(f"Model loaded from: {most_recent_model_file}")
